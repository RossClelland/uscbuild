...
###   uscoincore.org and uscoin.org and compares them.
...
WORKINGDIR="/tmp/uscoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://uscoincore.org"
HOST2="https://uscoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="uscoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'uscoin-',
...
if [ $? -ne 0 ]; then
   echo "uscoin.org failed to provide signature file, but uscoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "uscoin.org and uscoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Uscoin Core binary release signing key installed?"
   fi
