...
The RPM spec file provided here is for Uscoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://uscoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Uscoin git repository](https://github.com/uscoin/uscoin) but are not part of
...
Currently only `Source10` is used, for the example `uscoin.conf` file.
...
in the uscoin git repository. At present only an SVG version of the Uscoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the uscoin
...
needed when 0.12.1 is released, a proper fix is already in the Uscoin git
master branch.
...
idea to build Uscoin Core against a static version of that library compiled
...
The `uscoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `uscoin-server` package. At this point, most RPM based
...
The `uscoin-server` package should have SELinux support. How to properly do
...
Uscoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/uscoin/). He has been
packaging Uscoin for Fedora at least since 2012.
...
4. I renamed the `uscoin` package that contains the Qt GUI to `uscoin-core` as
that appears to be how the general population refers to it, in contrast to
`uscoin-xt` or `uscoin-classic`. I wanted to make sure the general population
