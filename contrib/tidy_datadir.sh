#!/bin/bash
# Copyright (c) 2013 The Uscoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Uscoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Uscoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Uscoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Uscoin 0.7 datadir."
    ;;
  3)
    echo "Detected Uscoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Uscoin 0.8 datadir."
    ;;
esac
